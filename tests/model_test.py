...
class VAEModelTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model = vae.VAEModel(self.output_size, self.meta_size, samples=2)
        self.batch_size = 32
    ...
    def train_test(self):
        ...
        xs = np.random.normal(size=(32, self.output_size + self.meta_size)).astype('float32')
        ys = np.random.normal(size=(32, self.output_size)).astype('float32')

        losses = []
        for _ in xrange(EPOCHS):
            losses.append(self.model._train_fn(xs, ys))
    ...
    def predict_test(self):
        ...
        xhat, logvar = self.model._predict_fn(xs)
